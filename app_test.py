...
def handle_start_timer():
    if not timer["running"]:
        ...
        timer["running"] = True
        emit('timer_started', {"start": timer["start"]}, broadcast=True)
        
@socketio.on('stop_timer')
def handle_stop_timer():
    if timer["running"]:
        elapsed = int(time.time()) - timer["start"]
        timer["running"] = False
        emit('timer_stopped', {"elapsed": elapsed}, broadcast=True)
...
def handle_time_update():
    if timer["running"]:
        ...
        emit('time_update', {"elapsed": elapsed})
